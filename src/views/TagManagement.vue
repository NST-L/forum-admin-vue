  ...
  <div class="tag-management">
    ...
    <div class="search-filter">
      <div class="filter-bar">
        <input class="input" v-model="searchForm.tag_name" placeholder="搜索标签名称" @input="handleSearch" />
      ...
      <table class="user-table card-style">
        ...
        <tbody v-if="tags.length > 0">
          <tr v-for="tag in tags" :key="tag.tag_id">
            ...
            <td>{{ tag.tag_name }}</td>
            <td>
              <button class="status-btn" @click="openEditDialog(tag)">编辑</button>
              <button class="status-btn disable" @click="handleDelete(tag)">删除</button>
            </td>
          </tr>
        </tbody>
        <tbody v-else>
    ...
    <DeleteConfirmDialog
      ...
      title="提示"
      :message="`确定要删除标签「${currentDeleteTag?.tag_name}」吗？`"
...
import { getTagList, addTag, updateTag, deleteTag } from '@/api/tagManagement';
  ...
  getTagList(requestParams).then(res => {
    if (res.data && res.data.success) {
      if (res.data.data && res.data.data.list) {
        // 处理标签数据
        const processedTags = res.data.data.list.map(tag => {
          return {
        ...
        if (tags.value.length === 0 && searchForm.page_num > 1) {
...
function openEditDialog(row) {
  ...
  dialogForm.tag_id = row.tag_id;
  dialogForm.tag_name = row.tag_name;
  dialogVisible.value = true;
}
...
function handleDialogConfirm(formData) {
  if (formData.tag_id) {
    ...
    // 新增
    addTag({ tag_name: formData.tag_name }).then(res => {
      if (res.data && res.data.success) {
...
function handleDeleteConfirm() {
  ...
  deleteTag(currentDeleteTag.value).then(res => {
    if (res.data && (res.data.success || res.data.code === 200)) {
      ElMessage.success('删除成功');
      deleteDialogVisible.value = false;
      
      // 如果删除后当前页没有数据了，且不是第一页，则跳转到上一页
      const currentPageItemCount = tags.value.length;
