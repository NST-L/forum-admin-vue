<template>
  <div class="user-violation">
    ...
    <!-- 举报卡片列表 -->
    <div v-if="filteredUsers.length === 0" class="empty-container">
    ...
    <div v-else class="report-list">
      <ViolationCard
        ...
        :key="user.reportId"
        :reportedContent="user.content || user.username"
        :reportedUsername="user.username"
        :avatarUrl="user.avatarUrl"
        :reporterId="user.reporterId"
        :reasonText="user.reasonTypeName || getReasonText(user.reasonType)"
        :reportTime="formatDate(user.ctime)"
        :status="user.status"
        :statusText="user.statusName || getStatusText(user.status)"
        :processorId="user.processorId"
        :processResult="user.processResult"
        :processTime="formatProcessTime(user.mtime, user.status)"
...
const fetchReportReasons = async () => {
  try {
    const response = await getReportReasons('USER')
    ...
    if (response.data.code === 0) {
      ...
      users.value = rawList.map(item => {
        return {
          ...
          // 被举报用户信息
          username: item.username, // 被举报用户的用户名
          avatarUrl: item.avatarUrl, // 被举报用户的头像
          componentId: item.componentId,
          content: item.content, // 被举报用户的内容
          // 举报人信息
      ...
      console.warn('[UserViolation] 接口返回错误:', response.data)
      ElMessage.error(response.data.message || '获取用户举报数据失败')
    }
  } catch (error) {
    console.error('获取用户违规举报数据失败:', error)
    ElMessage.error('获取用户举报数据失败，请稍后重试')
  } finally {
    loading.value = false
  }
}
...
const filteredUsers = computed(() => {
  return users.value.filter(user => {
    ...
    const matchReason = selectedReason.value === '' || user.reasonType === selectedReason.value
...
const processDialogData = computed(() => {
  ...
  return {
    id: currentUser.value.reportId, // 举报业务ID
    componentId: currentUser.value.componentId, // 被举报用户ID
    content: currentUser.value.content, // 被举报用户的具体内容
    username: currentUser.value.username, // 被举报用户名
    avatarUrl: currentUser.value.avatarUrl, // 被举报用户头像
    reporterId: currentUser.value.reporterId, // 举报人ID
    reporterName: currentUser.value.reporter, // 举报人名称
    reasonText: getReasonText(currentUser.value.reasonType) || reasonText(currentUser.value.reason),
    reportTime: formatDate(currentUser.value.ctime)
  }
})

// 为详情弹窗准备数据
const detailDialogData = computed(() => {
  if (!currentDetailUser.value) return null
  return {
    id: currentDetailUser.value.reportId, // 举报业务ID
    componentId: currentDetailUser.value.componentId, // 被举报用户ID
    content: currentDetailUser.value.content, // 被举报用户的具体内容
    username: currentDetailUser.value.username, // 被举报用户名
    avatarUrl: currentDetailUser.value.avatarUrl, // 被举报用户头像
    reporterId: currentDetailUser.value.reporterId, // 举报人ID
    reporterName: currentDetailUser.value.reporter, // 举报人名称
    reasonText: getReasonText(currentDetailUser.value.reasonType) || reasonText(currentDetailUser.value.reason),
    reportTime: formatDate(currentDetailUser.value.ctime),
    statusText: getStatusText(currentDetailUser.value.status),
    processorId: currentDetailUser.value.processorId,
    processResult: currentDetailUser.value.processResult,
    processTime: formatProcessTime(currentDetailUser.value.mtime, currentDetailUser.value.status)
  }
})
...
async function handleBanUser(processOpinion) {
  if (!currentUser.value || !processOpinion) {
  ...
  try {
    ...
    const res = await handleViolation('users', currentUser.value.reportId, 'ban', processOpinion)
    if (res.data.success) {
      // 本地立即变更
      const idx = users.value.findIndex(u => u.id === currentUser.value.id)
      if (idx !== -1) users.value[idx].status = 1
      ElMessage.success('用户封禁成功！举报已处理')
      setTimeout(() => {
        closeDialog()
        fetchUsers() // 异步刷新
      }, 800)
    } else {
      ElMessage.error(res.data.message || '用户封禁失败')
    }
  } catch (e) {
    ElMessage.error('用户封禁失败，请稍后重试')
  } finally {
...
async function handleRejectReport(processOpinion) {
  ...
  try {
    // 传递 reportId 而不是被举报内容的ID
    const res = await handleViolation('users', currentUser.value.reportId, 'reject', processOpinion)
    if (res.data.success) {
      // 本地立即变更
      const idx = users.value.findIndex(u => u.id === currentUser.value.id)
